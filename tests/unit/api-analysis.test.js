  ...
  it('should handle PR #33 via GitHub API (75 files, +24k lines)', async () => {
    ...
    while (hasMore && page <= 10) {
      const response = await fetch(
        `https://api.github.com/repos/jmalicki/arsync/pulls/33/files?per_page=100&page=${page}`
      );
    ...
    for (const [lang, langStats] of stats.entries()) {
      console.log(
        `   ${lang}: ${langStats.files} files, +${langStats.added} -${langStats.removed}`
      );
    }
    console.log(
    ...
    if (prData.additions !== totals.totalAdded) {
      console.log(
        `   ℹ️  Discrepancy: ${prData.additions - totals.totalAdded} additions difference`
      );
