...
async function investigateGitattributes() {
  ...
  const investigation = await page.evaluate(() => {
    ...
    containers.forEach((container, idx) => {
      ...
      const linguistAttrs = Object.keys(allAttrs).filter(k => 
        k.includes('linguist') || k.includes('generated') || k.includes('vendored')
      );
      ...
      const linguistClasses = classes.filter(c => 
        c.includes('generated') || c.includes('vendored') || c.includes('linguist')
      );
    ...
    const metaTags = document.querySelectorAll('meta[name*="linguist"], meta[name*="gitattributes"]');
